...
`
	...
	r, err := db.Exec(sql, t.StuId, t.Title, t.Description, t.Deadline)
	if err != nil {
...
`
	...
	var ts []*model.Topic
	rows, err := db.Query(sql /*enum.TOPIC_REVIEW_PASS, */, limitOne, size)
	if err != nil {
...
`
	//var count int
	if err := db.Get(&total, sql); err != nil {
...
`
	var ts []*model.Topic
	rows, err := db.Query(sql, enum.TOPIC_REVIEW_PASS, title)
	if err != nil {
...
`
	var t model.Topic
	rows, err := db.Query(sql, enum.TOPIC_REVIEW_PASS, id)
	if err != nil {
		logrus.Warningf("%s: %s", errno.MysqlSelectNoData.Error(), err.Error())
		return nil, errno.MysqlSelectNoData
	}
