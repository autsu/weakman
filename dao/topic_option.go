...
`
	...
	for _, option := range o {
		r, err := db.Exec(sql, option.TopicId, option.OptionContent)
		if err != nil {
...
`
	...
	if err := db.Select(&os, sql, topicId); err != nil {
...
`
	...
	if err := db.Get(&o, sql, id); err != nil {
...
where id = ?
`
	_, err = db.Exec(sql, num32, optionId)
	if err != nil {
...
`
	...
	if err := db.Select(&participantName, sql, id); err != nil {
